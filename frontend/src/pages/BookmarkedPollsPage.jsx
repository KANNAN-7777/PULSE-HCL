...
export default function BookmarkedPollsPage() {
  ...
  const loadBookmarkedPolls = useCallback(async () => {
    try {
      setLoading(true);
      setError("");

      const response = await api.get("/polls/bookmarked");
      ...
      console.error("Saved Polls Error:", err);

      setError(
        err.response?.data?.message ||
          err.response?.data?.error ||
          "Failed to load saved polls."
      );
    } finally {
      setLoading(false);
    }
  ...
  const removeBookmark = async (pollId) => {
    if (!pollId) {
      return;
    }
    ...
    try {
      ...
      await api.delete(`/polls/${pollId}/bookmark`);

      setPolls((currentPolls) =>
        currentPolls.filter((item) => {
          const id = item?.poll?.id || item?.poll?._id;
          return id !== pollId;
        })
      );
    } catch (err) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-[#9CA3AF]">
                  Polls you have saved for later.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="p-6">
              <div className="flex items-start gap-4">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-red-500/20 bg-red-500/10">
                  <BookmarkX className="h-5 w-5 text-red-400" />
                </div>

                <div>
                  <p className="text-sm font-bold text-red-400">
                    Unable to load saved polls
                  </p>
            ...
            <div className="relative mt-6 overflow-hidden rounded-3xl border border-[#292929] bg-[#111111] p-10 text-center shadow-[0_20px_60px_rgba(0,0,0,0.25)] animate-[fadeInUp_0.45s_ease-out]">
              ...
              <div className="relative">
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-[#FFD21F]/20 bg-[#FFD21F]/10">
                  <Bookmark className="h-8 w-8 text-[#FFD21F]" />
                </div>
          ...
          polls.length > 0 && (
            <div className="mt-6 space-y-4">
              {polls.map((item, index) => {
                ...
                return (
                  <div
                    key={`${pollId || "poll"}-${index}`}
                    className="group relative overflow-hidden rounded-3xl border border-[#292929] bg-[#111111] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.22)] transition duration-500 hover:-translate-y-1 hover:border-[#FFD21F]/20 hover:shadow-[0_25px_65px_rgba(0,0,0,0.30)] sm:p-6"
                    style={{
                      ...
                      {options.length > 0 && (
                        <div className="mt-6">
                          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#9CA3AF]">
                            ...
                            {options.map(
                              (option) => {
                                ...
                                return (
                                  <div
                                    key={option.id}
                                    className="rounded-2xl border border-[#292929] bg-[#090909] p-3 transition duration-300 hover:border-[#3A3A3A] hover:bg-[#0D0D0D]"
                                  >
                                    <div className="mb-2 flex items-center justify-between gap-3">
                                      <span className="min-w-0 truncate text-sm font-semibold text-[#D1D5DB]">
                                        {option.text}
                                      </span>
                                    ...
                                    <div className="h-2 overflow-hidden rounded-full bg-[#292929]">
                                      <div
                                        className="h-full rounded-full bg-gradient-to-r from-[#FFD21F] to-[#FFE66D] shadow-[0_0_15px_rgba(255,210,31,0.18)] transition-all duration-700"
                                        style={{
                                          width: `${Math.min(
                                            Math.max(
                                              percentage,
                      ...
                      <div className="mt-5 flex flex-col gap-3 border-t border-[#292929] pt-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                          {pollId && (
                            <Link
                              to={`/poll/${pollId}`}
                              className="group/view inline-flex items-center gap-1 text-sm font-bold text-[#FFD21F] transition hover:text-[#FFE66D]"
                            >
                              View Poll
                          ...
                          {pollId && (
                            <button
                              ...
                              className="group/remove inline-flex items-center gap-1.5 text-sm font-bold text-red-400 transition hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              {removingId ===
                              pollId ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                              ) : (
                                <BookmarkX className="h-4 w-4 transition-transform duration-300 group-hover/remove:scale-110" />
                              )}
